...
public class AlphaBeta {
        ...
        for (int i = 0; i < 15; i++) {
            for (int j = 0; j < 15; j++) {
                if (board[i][j] == 0 && caroChessUtils.checkLegalMove(i, j)) {
                    board[i][j] = player;
                    int score = alphaBeta(board, hard, Integer.MIN_VALUE, Integer.MAX_VALUE, false, (player == 1) ? 2 : 1);
                    board[i][j] = 0;
    ...
    private int alphaBeta(int[][] board, int depth, int alpha, int beta, boolean isMaximizing, int player) {
        ...
        if (isMaximizing) {
            ...
            for (int i = 0; i < 15; i++) {
                for (int j = 0; j < 15; j++) {
                    if (board[i][j] == 0 && caroChessUtils.checkLegalMove(i, j)) {
                        board[i][j] = player;
                        int score = alphaBeta(board, depth - 1, alpha, beta, false, player);
                        board[i][j] = 0;
            ...
            for (int i = 0; i < 15; i++) {
                for (int j = 0; j < 15; j++) {
                    if (board[i][j] == 0 && caroChessUtils.checkLegalMove(i, j)) {
                        ...
                        int score = alphaBeta(board, depth - 1, alpha, beta, true, opponentPlayer);
                        board[i][j] = 0;
